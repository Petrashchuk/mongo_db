...
module.exports.show_statistic = async function (req, res) {
            ...
            "yes": [
                {$match: {value: true}},
                {
                    $bucket:
                        {
                            groupBy: "$user.age",
                            boundaries: [0, 20, 30, 40, 50, 61],
                            default: "other",
                            output:
                                {
                                    "total": {$sum: 1},
                                    "male": {$sum: {$cond: {if: {$eq: ["$user.gender", "male"]}, then: 1, else: 0}}},
                                    "female": {$sum: {$cond: {if: {$eq: ["$user.gender", "female"]}, then: 1, else: 0}}}
                                }
                        }
                }
            ],
    ...
    //
    //         {$group: {_id: {userGender: "$userGender", range: "$range"}, count: {$sum: 1}}}
    //         // {$group: {_id: {userGender: "$userGender", userNationality: "$user.nationality"}, count: {$sum: 1}}}
    //     ]
    // });
    //
